...
# 第一次更换新的账号登陆时需要输入验证码，会导致程序运行错误，暂时还没有解决这一问题。但第二次登陆不用输入验证码，可以正常运行
# 发布投票选择投票截止时间时可能会出错（第80行下方代码），可能需要自己审查浏览器定位元素更改代码
# 导入的用户账号密码文件名为 account_data.csv 表头：用户  账号 密码


from 易班.yanzhengma import Recognise
...
def login_yiban(account_str, password_str):
    ...
    src = browser.find_element_by_class_name('captcha').get_attribute('src')
    # 发送请求获取响应
    response = requests.get(url=src, headers=headers,verify=False,timeout=5)
    # 保存图片
    localPath  = "D:\\验证码\\"
    ...
    # with open(localPath+str(localtime)+".jpg", 'wb') as f:
    ...
    im = ImagePIL.open(localPath+str(localtime)+".jpg")
    im.save(r'D:\\验证码\\'+str(localtime)+'handle.jpg',dpi=(96.0,96.0))
...
def publish_votes(user_name, n):
    for j in range(n):
        ...
        browser.find_element_by_xpath('/html/body/main/div/section/div[1]/div/ul[1]/li[1]/div[1]/input').send_keys(
            vote_options[randint(0, 17)]*randint(1, 3))

        time.sleep(1)
        browser.find_element_by_xpath('/html/body/main/div/section/div[1]/div/ul[1]/li[2]/div/input').send_keys(
            vote_options[randint(0, 17)]*randint(1, 3))

        time.sleep(1)
        browser.find_element_by_xpath('/html/body/main/div/section/div[1]/div/ul[1]/li[3]/div/input').send_keys(
            vote_options[randint(0, 17)]*randint(1, 3))
        ...
        # browser.find_element_by_xpath('/html/body/main/div/section/div[1]/div/p[4]/div/div[3]/table/tbody/tr[4]/td[7]')

        # /html/body/main/div/section/div[1]/div/p[4]/div/div[3]/table/tbody/tr[5]/td[5]
        # /html/body/main/div/section/div[1]/div/p[4]/div/div[2]/table/tbody/tr/td/span[23]

        # 选择截止日期这部分会出现问题  暂时没有提供很好的的解决方法
        browser.find_element_by_xpath(
            '/html/body/main/div/section/div[1]/div/p[4]/div/div[3]/table/tbody/tr[5]/td[5]').click()   # 选择日期
        browser.find_element_by_xpath(
            '/html/body/main/div/section/div[1]/div/p[4]/div/div[2]/table/tbody/tr/td/span[23]').click()  # 选择截止时间
        browser.find_element_by_xpath(
            '/html/body/main/div/section/div[1]/div/p[4]/div/div[1]/table/tbody/tr/td/span[12]').click()  # 选择截止时间
        print("选择时间完成")
...
if action == '发布投票':
    ...
    user_list, account_list, password_list = load_account('D:\\account_data.csv')
